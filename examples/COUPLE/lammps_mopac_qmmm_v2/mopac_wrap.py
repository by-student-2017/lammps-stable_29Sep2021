...
vaspcmd_scf = "/mnt/d/mopac-22.1.1/bin/mopac in_scf.mop"
...
def poscar_write(poscar,QMatoms,ntypes,types,coords,box,types_symbol):
  ...
  psnew_scf = open("in_scf.mop",'w')
  psnew = open("in.mop",'w')
  
  # header (keywords and comment line)
  
  psnew_scf.write(psold[0].strip("\n")+" DENOUT CAMP CYCLES=1000 \n") # comment line
  psnew_scf.write(psold[1]) # comment line
  psnew_scf.write(psold[2]) # comment line
  #
  psnew.write(psold[0].strip("\n")+" OLDENS 1SCF GRAD AUX QMMM \n") # comment line
...
while 1:
  ...
  #print("\nLaunching DFTB+ ...")
  #print(vaspcmd)
  subprocess.check_output(vaspcmd_scf,stderr=subprocess.STDOUT,shell=True)
  
  subprocess.check_output("cp in_scf.den in.den",stderr=subprocess.STDOUT,shell=True)
