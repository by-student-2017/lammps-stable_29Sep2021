...
def vasp_setup(poscar): # poscar = gen format
  ...
  if sttype == "S":
    words = ps[QMatoms+3].split()
    xbox = float(words[0])
    words = ps[QMatoms+4].split()
    ybox = float(words[1])
    words = ps[QMatoms+5].split()
    zbox = float(words[2])
    box = [xbox,ybox,zbox]
  else:
    box = [0.0, 0.0, 0.0]

  return QMatoms,ntypes,box
  
# -------------------------------------
# write a new POSCAR file for VASP

def poscar_write(poscar,QMatoms,ntypes,types,coords,box):

  psold = open(poscar,'r').readlines()
  psnew = open("geometry.gen",'w')
...
def charge_write(QMatoms,natoms,ntypes,types,coords,lmp_charges):

  psnew = open("charges.dat",'w')
...
QMatoms = 0
QMatoms,ntypes,box = vasp_setup(poscar_template)
...
while 1:
  ...
  if msgID == SETUP:
    ...
    for field in fieldID:
      if field == DIM:
        ...
        natoms = natoms_recv
        subprocess.check_output("sed 's/\#Records/Records = "+str(natoms_recv-QMatoms)+"/g' dftb_in_tmp.hsd > dftb_in.hsd",stderr=subprocess.STDOUT,shell=True)
  ...
  poscar_write(poscar_template,QMatoms,ntypes,types,coords,box)
  
  # create charges.dat file
  
  charge_write(QMatoms,natoms,ntypes,types,coords,lmp_charges)
